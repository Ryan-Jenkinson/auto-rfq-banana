...
- **Consolidation analysis** with carve-outs (Ryan's actual award strategy — pick one supplier, identify exceptions where another supplier saves enough to justify a separate PO). Carve-out logic is **OR** between two thresholds: `carve_out_min_savings_pct` (default 20% — fires on long-tail anomalies) and `carve_out_min_savings_annual_dollar` (default $3,000/yr — fires on high-volume rate plays). Either threshold firing carves the item. Each carve-out records `carve_rule_fired` ∈ {`PCT`, `DOLLAR`, `BOTH`} so the matrix tooltip and Decision Log can explain *which* rule justified the carve. UOM-suspect carve-outs are flagged separately and excluded from counted savings.
- **Comparison matrix** (items × suppliers) with recommendation chips (5-tier: ACCEPT / PUSH_BACK / ASK_CLARIFICATION / EXCLUDE / MANUAL_REVIEW), every recommendation carries a concrete reason string. Columns: R2 / Item (description — Coupa convention) / Part # / Last order date / Qty 24mo / Last $/ea (amber-tinted, the historical baseline; vertical accent separator BEFORE this column AND after the last supplier column so the price block reads as one grouped section) / N supplier prices / Cov / Recommendation. All headers sortable; rows click-selectable; cell click → per-item modal. Matrix has a click-target filter system: bid-coverage KPI tiles (3+ / 2 / 1 / 0 / Outliers), recommendation chips, supplier intake cards, and consolidation candidate rows all toggle filters that narrow the matrix below; per-cell click drills into the per-item modal. A pill bar at the top of the matrix surfaces active filters with × buttons + a "clear all" link. **Round 2 selection toolbar** above the matrix: leftmost R2 column adds a per-row checkbox; quick-selects (PUSH_BACK / wide-spread / single-source) prefill common patterns; selection count + estimated 24-mo $ coverage updates live; GENERATE button opens a per-supplier batch dialog. Cells on bids that have a `round_history` (i.e., R2/Rn overwrote the R1 bid) carry a small `R2 ↓ %` badge with the prior price in the tooltip. Selection persists via `_STATE["round2_selection"]`.
...
**Step 4 progressive-disclosure layout (post-redesign).** The page is one scrollable surface with 4 vertical bands: [1] INTAKE (always visible), [2] PARTIAL-DATA BANNER (only when fewer suppliers loaded than the outbound RFQs we generated), [3] HEADLINE CARD (system recommendation + chip strip + manual-override pips + reset-to-auto + inline snapshot strip), [4] COVERAGE KPIs + COMPARISON MATRIX, [5] DRAWER STACK (Carve-out details / Round 2 / Advanced — each collapsed by default with a $ teaser). The Saved Scenarios drawer was removed in favor of an inline snapshot strip in the headline card — both surfaces overlapped (chip = explore live; drawer = save), so they were combined into one. Click `📌 Save snapshot` to bookmark the active state; saved snapshots show as inline pills with action popovers (letters / internal summary / Decision Log / delete). Default chip is `consolidate_to` matching Ryan's actual award playbook; default consolidate target is the top consolidation candidate (ranked coverage-first then aggregate price, so a supplier who only quotes 1 cheap item can't "win" by virtue of low aggregate). Chip flips re-render the headline from a cached payload without a Python round-trip; only the consolidate-to dropdown triggers a re-fetch.

**Apples-to-apples savings math.** `_evaluate_scenario` returns `covered_*` (items that got an award) and `uncovered_*` (items where NO supplier provided a priced bid) as separate fields. The headline savings number is `covered_savings_total = covered_historical_total − covered_award_total` — items with no bid never inflate it. The uncovered count + their historical $ surface separately as a follow-up signal ("⚠ N items uncovered, $X historical, NOT in savings"). All 5 strategies use this math; the carve-out drawer's "$X NOT counted" line agrees with the headline to the dollar. UOM-suspect carve guard: in `_evaluate_scenario`'s `consolidate_to` branch, when the cheapest other bid is `UOM_DISC` OR price-ratio target/chosen >= `uom_suspect_ratio`, the carve doesn't fire — the item stays at the consolidate target's price and is recorded as `n_suspect_carves_held`.

**Universal row select + sortable columns.** Tables that opt in via `data-table-id="..." data-rowselect="1" data-sortable="1"` get persistent click-to-select rows (cyan stripe + bg, click another row to reselect, click same row to deselect) and click-to-sort headers (ASC → DESC → revert). Per-table memory in module-level Maps survives re-renders. Headers use `data-sort-key="..." data-sort-type="num|str|date"` + `class="sortable"`. Cells use `data-sort-cell="key" data-sort-value="..."` OR rows use `data-sort-<key>="..."`. Wired today: comparison matrix, consolidation candidates, verified carve-outs (with double-click-row → per-item modal). Per-item modal source row gets `.modal-open-source` class while modal is open across every matching table; existing `.last-viewed-row` fade takes over on close.

**Carve-out OR-rule.** Carves fire on `pct_savings >= carve_out_min_savings_pct` OR `annual_dollar_savings >= carve_out_min_savings_annual_dollar`. Defaults: 20% / $3,000/yr. Both editable in the ⚙ THRESHOLDS panel. Each carve_out records `carve_rule_fired` ∈ {`PCT`, `DOLLAR`, `BOTH`}. Annual savings is computed as `(winner_price - other_price) × (qty_24mo / 2.0)` — halving 24-month qty gives the run-rate annual figure.

**Decision Summary xlsx (legal-hold record).** Generated via the 📋 button at the bottom of step 4. 7 tabs: Executive_Summary (prose narrative + headline numbers) / Settings_Thresholds (every threshold + scenario param at decision time) / Analyst_Actions (locks · exclusions · UOM resolutions · follow-up flags · scenarios — quantifies user work) / System_Flags (recommendations · outliers · UOM_DISC · partial coverage — quantifies app work) / **CostAvoid_vs_Savings** (the two reporting numbers tracked SEPARATELY: cost avoidance = active vs historical baseline; savings uplift = active vs no-touch auto baseline; both feed leadership reporting per Ryan's guidance to track both) / FollowUp_Items (markable list of SKUs flagged for post-award double-check via `flag_item_for_follow_up`) / Decision_Log_Timeline (full audit_log). Every tab carries the "INTERNAL — NEVER FORWARD — RETAIN PER LEGAL HOLD" banner.
...
| `compute_headline_strategies(consolidate_supplier=None)` | Pre-compute all 5 strategies' totals for the chip-strip switcher in step 4's headline card. Returns supplier_primary, award_total, savings_total, n_carved per strategy + default chip + manual_overrides counters + active thresholds. |
| `_summarize_eval_for_headline(evaluated, consolidate_supplier=None)` | Compress an `_evaluate_scenario` result into the slim shape the headline card needs. Pure (no state mutation). |
| `reset_to_auto()` | Clear analyst overrides (item_locks + item_exclusions + uom_annotations), recompute affected aggregates, audit-log the action. UI button: ↺ Reset to auto in the headline card. |
| `flag_item_for_follow_up(item_num, note)` / `resolve_item_follow_up(item_num, note)` / `list_follow_up_flags()` | Per-SKU "needs double-check after award" flag. Resolved flags stay on the record (not deleted). Surfaced in Decision Summary Tab 6. |
| `compute_decision_summary_metrics(scenario_name=None)` | Aggregated payload for the legal-hold Decision Summary: cost avoidance vs historical AND savings uplift vs no-touch auto baseline (tracked separately for leadership reporting), analyst work counters, system flag counters, follow-ups, audit log, threshold snapshot. |
| `_build_decision_narrative(metrics)` | 5-paragraph prose summary auto-generated from the metrics dict. Pure string formatting. |
| `gen_decision_summary_xlsx(scenario_name=None, rfq_id="")` | The 7-tab Decision Summary xlsx — Executive_Summary / Settings_Thresholds / Analyst_Actions / System_Flags / CostAvoid_vs_Savings / FollowUp_Items / Decision_Log_Timeline. Banner row 1 "INTERNAL — NEVER FORWARD — RETAIN PER LEGAL HOLD". Filename: `DecisionSummary_auto-rfq-banana_<stem>_<YYYY-MM-DD>.xlsx`. Generated via the 📋 Decision Summary xlsx button at the bottom of step 4. |
