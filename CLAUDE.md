...
**Step 4 progressive-disclosure layout (post-redesign).** The page is one scrollable surface with 5 vertical bands: [1] INTAKE (always visible), [2] PARTIAL-DATA BANNER (only when fewer suppliers loaded than the outbound RFQs we generated), [3] HEADLINE CARD ("→ AWARD TO <SUPPLIER>" + 5-chip strategy switcher [Lowest Price · Lowest Qualified · Consolidate to ▾ · Incumbent Preferred · Manual] + manual-override pips [N locks · M outlier-exclusions · K UOM resolutions] + ↺ Reset-to-auto), [4] COVERAGE KPIs + COMPARISON MATRIX (the working table), [5] DRAWER STACK (Hybrid carve-outs / Round 2 / Saved scenarios / Advanced — each collapsed by default with a $ teaser). Default chip is `consolidate_to` matching Ryan's actual award playbook; default consolidate target is the top consolidation candidate (now ranked coverage-first then aggregate price, so a supplier who only quotes 1 cheap item can't "win" by virtue of low aggregate). Chip flips re-render the headline from a cached payload without a Python round-trip; only the consolidate-to dropdown triggers a re-fetch.
